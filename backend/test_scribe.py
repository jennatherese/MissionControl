import sys
sys.path.insert(0, '.')
from agents.scribe_agent import _parse_locally
...
result = _parse_locally(transcript)
if result:
    print(f"\nFound {len(result)} tasks:")
    for t in result:
        print(f"  [{t['owner']}] {t['title']} | P{t['priority']} | {t['deadline'][:10]}")
else:
